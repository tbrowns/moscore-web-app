...
export function NewWorkspace({
  ...
  const uploadFile = async (clusterId: string) => {
    ...
    for (const file of files) {
      try {
        const dataIndices = fileData
          .map((f, index) => (f.name === file.name ? index : null))
          .filter((index) => index !== null);

        console.log("data:", fileData[dataIndices[0]]);
        ...
        await new Promise<string>((resolve, reject) => {
          uploadTask.on(
            ...
            (snapshot) => {
              ...
              fileData[dataIndices[0]].downloadProgress = Math.floor(progress);
  ...
  const handleSubmit = async () => {
    ...
    await createResource(clusterId);
  };
